    ...
    try {
      ...
      if (req["file"] && req["file"]?.path && retPost?.calibracao?.id) {
        fs.readFile(req["file"].path, async (fsErr, data) => {
          if (!fsErr)
            await salvarPdfCertificadoCalibracao(retPost.calibracao.id, data);
...
export const salvarCalibracao = async (
  ...
  if (id) {
    ...
    delete calibracao.pdfCertificado;
...
export const salvarPdfCertificadoCalibracao = async (
  calibracaoId: string,
  pdf: Buffer
) => {
