...
export default async function Indicadores(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      sqlResult = await prisma.$queryRaw(`select count(1) qtde from (${ConsultasComuns.vencimentosCalibracoes()}) as v
        where v.ativo = 1 and v.dias_vencer < 0 and ${(pessoaId ? `v.pessoa_id = '${pessoaId}'` : '1=1')}`);
      json.calibracoes_vencidas = sqlResult[0].qtde;

      // Quantidade de Calibrações a Vencer em 7 dias
      sqlResult = await prisma.$queryRaw(`select count(1) qtde from (${ConsultasComuns.vencimentosCalibracoes()}) as v
        where v.ativo = 1 and v.dias_vencer between 0 and 7 and ${(pessoaId ? `v.pessoa_id = '${pessoaId}'` : '1=1')}`);
      json.calibracoes_vencer_7 = sqlResult[0].qtde;
      
      // Quantidade de Calibrações a Vencer em 15 dias
      sqlResult = await prisma.$queryRaw(`select count(1) qtde from (${ConsultasComuns.vencimentosCalibracoes()}) as v
        where v.ativo = 1 and v.dias_vencer between 0 and 15 and ${(pessoaId ? `v.pessoa_id = '${pessoaId}'` : '1=1')}`);
      json.calibracoes_vencer_15 = sqlResult[0].qtde;

      // Quantidade de Calibrações a Vencer em 30 dias
      sqlResult = await prisma.$queryRaw(`select count(1) qtde from (${ConsultasComuns.vencimentosCalibracoes()}) as v
        where v.ativo = 1 and v.dias_vencer between 0 and 30 and ${(pessoaId ? `v.pessoa_id = '${pessoaId}'` : '1=1')}`);
