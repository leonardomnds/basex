...
const ListaClientes: NextPage<Props> = (props: Props) => {
  ...
  const getWhere = (csv: boolean = false) => {
    ...
    if (nome) where += ` and upper(trim(${csv ? 'clientes' : 'p'}.nome)) like '%${nome.trim().toUpperCase()}%'`;
    if (fantasia) where += ` and upper(trim(${csv ? 'clientes' : 'p'}.fantasia)) like '%${fantasia.trim().toUpperCase()}%'`;
    if (status === 0 || status === 1) where += ` and ${csv ? 'clientes' : 'p'}.ativo = ${status}`;

    if (dataCadastroInicial) where += ` and ${csv ? 'clientes' : 'p'}.data_cadastro >= '${format(dataCadastroInicial, 'yyyy-MM-dd')}'`;
    if (dataCadastroFinal) where += ` and ${csv ? 'clientes' : 'p'}.data_cadastro <= '${format(dataCadastroFinal, 'yyyy-MM-dd')}'`;
    ...
    if ((cidade || '') === '-') {
      ...
      where += ` and upper(trim(${csv ? 'clientes' : 'p'}.cidade)) = '${cidade.trim().toUpperCase()}'`;
    }

    if ((bairro || '') === '-') {
      where += ` and nullif(trim(${csv ? 'clientes' : 'p'}.bairro),'') is null`;
    } else if ((bairro || '').length > 1) {
      where += ` and upper(trim(${csv ? 'clientes' : 'p'}.bairro)) = '${bairro.trim().toUpperCase()}'`;
    }
        ...
        <Grid item xs={12} sm={8} md={6} lg={3}>
          <Box className={classes.btnBox}>
            ...
            <CustomButton
              label="Gerar PDF"
              icon={<PrintIcon/>}
              func={() => AbrirRelatorio(NomeRelatorio.listaClientes, getWhere())}
            />
          </Box>
        </Grid>
      </Grid>
  ...
  useEffect(() => {    
    const getData = async () => {
      const jsonBairros = [];
      try {
        const response = await api.get(`/estados/${estado}/cidades/${cidade.trim()}/bairros`);
...
export const getServerSideProps: GetServerSideProps = async ({ req, res }) => {
  ...
  from
    pessoas
  order by
    case when uf is null then 0 else 1 end,
    uf
  `);
  ...
  jsonEstados.map((e) => {
    if (e.uf === '-') {
      ...
      estados.push({ value: e.uf, text: `${e.uf} - ${listarEstados().filter((est) => est.uf === e.uf)[0].descricao}` })
    }
  });
