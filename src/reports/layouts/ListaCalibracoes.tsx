...
const ListaInstrumentos: NextPage<Props> = (props: Props) => {
  ...
  const gerarTabela = (dadosFiltrados: []) => {
    ...
    dadosFiltrados.map((i: any) => {
      celulas = [];
      celulas.push({ label: i.data_calibracao ? format(new Date(i.data_calibracao), 'dd/MM/yyyy') : '' });
  ...
  const getGruposInstrumentos = () => {
    ...
    dados.map((i: any) => {
      ...
      if (instrumentos.filter((ins: any) => ins.tag === i.tag && ins.descricao === i.descricao).length === 0) {
    ...
    instrumentos.map((i: any) => {
      gruposGerados.push(
        <View>
          ...
          {gerarTabela(dados.filter((v: any) => v.tag === i.tag && v.descricao === i.descricao) as [])}
        </View>
      );      
    });
