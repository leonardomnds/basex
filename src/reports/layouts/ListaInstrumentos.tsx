...
const ListaInstrumentos: NextPage<Props> = (props: Props) => {
  ...
  const gerarTabela = (dadosFiltrados: []) => {
    ...
    dadosFiltrados.map((i: any) => {
      ...
      celulas.push({
        label: i.ultima_calibracao ? FormatUtcDate(i.ultima_calibracao) : "",
      });
      celulas.push({
        label: i.vencimento_calibracao
  ...
  const getGruposClientes = () => {
    ...
    dados.map((c: any) => {
      ...
      if (
        clientes.filter((cli: any) => cli.codigo === c.codigo_cliente)
          .length === 0
      ) {
    ...
    clientes.map((c: any) => {
      gruposGerados.push(
        <View>
          ...
          {gerarTabela(
            dados.filter((v: any) => v.codigo_cliente === c.codigo) as []
          )}
        </View>
      );
    });
